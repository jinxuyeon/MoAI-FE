      ...
      <div className="container">
        <div className="inner-container">
          <aside className="profile-section">
            ...
            <label>별명</label>
            <input type="text" value={user.nickname || ""} onChange={(e) => setUser((prev) => ({ ...prev, nickname: e.target.value }))} />
            <button className="save-nickname-btn" onClick={handleSaveNickname}>저장</button>


            <label>자기소개</label>
            <textarea value={intro} placeholder="200자 이내로 입력하세요" onChange={(e) => setIntro(e.target.value)} />
          ...
          <main className="main-content">
            ...
            <div className="activity-tabs">
              <button className={activeTab === "posts" ? "active" : ""} onClick={() => setActiveTab("posts")}>작성글</button>
              <button className={activeTab === "comments" ? "active" : ""} onClick={() => setActiveTab("comments")}>작성댓글</button>
            </div>
            ...
            <div className="box checklist-box">
              ...
              <ul className="checklist-items">
                {checklist.map((item, index) => (
                  <li key={index} className={item.checked ? "checked" : ""} onClick={() => toggleItem(index)}>
          ...
          <aside className="board-section">
            <div className="search-box">
              <div className="search-input-wrapper">
                <input type="text" placeholder="전체 게시판의 글을 검색해보세요!" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
              </div>
            </div>
