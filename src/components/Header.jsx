...
import { FcAdvertising, FcGraduationCap, FcManager, FcRating, FcCollaboration, FcLock, FcSms, FcPaid } from "react-icons/fc";
...
function Header() {
  ...
  useEffect(() => {
    ...
    bodyScrollYRef.current = window.scrollY || document.documentElement.scrollTop || 0;
  ...
  const gnbData = [
    {
      ...
      subMenus: [
        { title: "전체 공지사항", desc: "모든 공지를 한곳에서 확인해요.", icon: <FcAdvertising />, to: "#" },
        { title: "학교 공지사항", desc: "교내 공지사항이에요.", icon: <FcGraduationCap />, to: "/main/community/notice_univ" },
        { title: "학과 공지사항", desc: "교내 공지사항이에요.", icon: <FcGraduationCap />, to: "/main/community/notice_dept" },
        { title: "조교알림", desc: "조교가 올린 학과 공지에요.", icon: <FcManager />, to: "/main/community/notice" },
      ],
    },
    {
      label: "커뮤니티",
      ...
      subMenus: [
        { title: "인기 게시판", desc: "지금 가장 인기있는 글들이에요.", icon: <FcRating />, to: "/main/community/popular" },
        { title: "자유 게시판", desc: "자유롭게 이야기를 나눠보세요.", icon: <FcCollaboration />, to: "/main/community/free" },
        { title: "비밀 게시판", desc: "하고 싶은 말, 익명으로 털어놔요.", icon: <FcLock />, to: "/main/community/secret" },
        { title: "후기 게시판", desc: "다양한 후기를 공유하는 곳이에요.", icon: <FcSms />, to: "/main/community/review" },
      ],
    },
    {
      label: "장터",
      ...
      subMenus: [
        { title: "책 장터", desc: "중고 책을 사고 팔 수 있어요.", icon: <FcPaid />, to: "/main/community/market" },
      ],
    },
    {
      label: "스터디룸",
          ...
          <div className="tooltip">
            <div className="gnb-sub">
              {menu.subMenus.map((sub, subIdx) => (
                <Link key={subIdx} to={sub.to} className="submenu-item">
                  <div className="submenu-icon">{sub.icon}</div>
                  <div className="submenu-content">
                    <div className="submenu-title">{sub.title}</div>
                    <div className="submenu-desc">{sub.desc}</div>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <div className="header-wrap">
            ...
            {!isMobile && (
              <div className="btn-wrap more-gap">
                <button aria-label="프로필" className="header-btn profile">
                  <MyProfile profileImageUrl={user?.profileThumbnails} />
                </button>
                <div className="profile-menu-list-area">
                  <ul className="profile-menu-list">
                    {profileMenuItems.map((item, index) => (
                      ...
                      >
                        <button className="profile-menu-btn">{item.label}</button>
                      </li>
                    ))}
                  </ul>
                </div>
